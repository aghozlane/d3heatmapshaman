function heatmap(selector, data, options) {
  ...
  var el = d3h.select(selector);

  var bbox = el.node().getBoundingClientRect();

  var Controller = function() {
    this._events = d3h.dispatch("highlight", "datapoint_hover", "transform");
  ...
  function colormap(svg, data, width, height) {
    ...
    var x = d3h.scale.linear().domain([0, cols]).range([0, width]);
    var y = d3h.scale.linear().domain([0, rows]).range([0, height]);
    var tip = d3h.tip()
    ...
    var brush = d3h.svg.brush()
        ...
        .on('brush', function() {
          ...
          d3h.select(this).call(brush.extent(extent));
        })
        .on('brushend', function() {
          ...
          brush.clear();
          d3h.select(this).call(brush).select(".brush .extent")
    ...
    brushG.select("rect.background")
        ...
        .on("mousemove", function() {
          var e = d3h.event;
          var offsetX = d3h.event.offsetX;
          var offsetY = d3h.event.offsetY;
          ...
          tip.show({col: col, row: row, label: label}).style({
            top: d3h.event.clientY + 15 + "px",
            left: d3h.event.clientX + 15 + "px",
            opacity: 0.9
          });
  ...
  function axisLabels(svg, data, rotated, width, height, padding) {
    ...
    var leaves;
    if (data.children) {
      leaves = d3h.layout.cluster().nodes(data)
    ...
    // Define scale, axis
    var scale = d3h.scale.ordinal()
        .domain(leaves)
        .rangeBands([0, rotated ? width : height]);
    var axis = d3h.svg.axis()
        .scale(scale)
  ...
  function dendrogram(svg, data, rotated, width, height, padding) {
    ...
    var x = d3h.scale.linear()
        .domain([data.height, 0])
        .range([topLineWidth/2, width-padding]);
    var y = d3h.scale.linear()
        .domain([0, height])
        .range([0, height]);
    
    var cluster = d3h.layout.cluster()
  ...
  var dispatcher = d3h.dispatch('hover', 'click');
  ...
  function on_col_label_mouseenter(e) {
    controller.highlight(+d3h.select(this).attr("index"), null);
  }
  function on_col_label_mouseleave(e) {
    controller.highlight(null, null);
  }
  function on_row_label_mouseenter(e) {
    controller.highlight(null, +d3h.select(this).attr("index"));
  }
