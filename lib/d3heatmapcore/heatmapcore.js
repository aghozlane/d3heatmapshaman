function heatmap(selector, data, options) {
  ...
  var el = d33.select(selector);

  var bbox = el.node().getBoundingClientRect();

  var Controller = function() {
    this._events = d33.dispatch("highlight", "datapoint_hover", "transform");
  ...
  function colormap(svg, data, width, height) {
    ...
    var x = d33.scale.linear().domain([0, cols]).range([0, width]);
    var y = d33.scale.linear().domain([0, rows]).range([0, height]);
    var tip = d33.tip()
    ...
    var brush = d33.svg.brush()
        ...
        .on('brush', function() {
          ...
          d33.select(this).call(brush.extent(extent));
        })
        .on('brushend', function() {
          ...
          brush.clear();
          d33.select(this).call(brush).select(".brush .extent")
    ...
    brushG.select("rect.background")
        ...
        .on("mousemove", function() {
          var e = d33.event;
          var offsetX = d33.event.offsetX;
          var offsetY = d33.event.offsetY;
          ...
          tip.show({col: col, row: row, label: label}).style({
            top: d33.event.clientY + 15 + "px",
            left: d33.event.clientX + 15 + "px",
            opacity: 0.9
          });
  ...
  function axisLabels(svg, data, rotated, width, height, padding) {
    ...
    var leaves;
    if (data.children) {
      leaves = d33.layout.cluster().nodes(data)
    ...
    // Define scale, axis
    var scale = d33.scale.ordinal()
        .domain(leaves)
        .rangeBands([0, rotated ? width : height]);
    var axis = d33.svg.axis()
        .scale(scale)
  ...
  function dendrogram(svg, data, rotated, width, height, padding) {
    ...
    var x = d33.scale.linear()
        .domain([data.height, 0])
        .range([topLineWidth/2, width-padding]);
    var y = d33.scale.linear()
        .domain([0, height])
        .range([0, height]);
    
    var cluster = d33.layout.cluster()
  ...
  var dispatcher = d33.dispatch('hover', 'click');
  ...
  function on_col_label_mouseenter(e) {
    controller.highlight(+d33.select(this).attr("index"), null);
  }
  function on_col_label_mouseleave(e) {
    controller.highlight(null, null);
  }
  function on_row_label_mouseenter(e) {
    controller.highlight(null, +d33.select(this).attr("index"));
  }
