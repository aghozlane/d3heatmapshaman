!function() {
  ...
  d33.descending = function(a, b) {
    return b < a ? -1 : b > a ? 1 : b >= a ? 0 : NaN;
  };
  d33.min = function(array, f) {
  ...
  d33.quantile = function(values, p) {
    var H = (values.length - 1) * p + 1, h = Math.floor(H), v = +values[h - 1], e = H - h;
    return e ? v + e * (values[h] - v) : v;
  };
  d33.median = function(array, f) {
    ...
    if (numbers.length) return d33.quantile(numbers.sort(d3_ascending), .5);
  };
  d33.variance = function(array, f) {
  ...
  d33.deviation = function() {
    var v = d33.variance.apply(this, arguments);
  ...
  d33.zip = function() {
    ...
    for (var i = -1, m = d33.min(arguments, d3_zipLength), zips = new Array(m); ++i < m; ) {
  ...
  d33.transpose = function(matrix) {
    return d33.zip.apply(d3, matrix);
  };
  d33.keys = function(map) {
    var keys = [];
    for (var key in map) keys.push(key);
    return keys;
  };
  d33.values = function(map) {
    var values = [];
    for (var key in map) values.push(map[key]);
    return values;
  };
  d33.entries = function(map) {
    var entries = [];
  ...
  var abs = Math.abs;
  d33.range = function(start, stop, step) {
  ...
  d33.nest = function() {
    ...
    nest.entries = function(array) {
      return entries(map(d33.map, array, 0), 0);
    };
    nest.key = function(d) {
      keys.push(d);
      return nest;
  ...
  function d3_selection_creator(name) {
    return typeof name === "function" ? name : (name = d33.ns.qualify(name)).local ? function() {
  ...
  var d3_selectionRoot = d33.select(d3_documentElement);
  ...
  function d3_selection_on(type, listener, capture) {
    ...
    function removeAll() {
      var re = new RegExp("^__on([^.]+)" + d33.requote(type) + "$"), match;
  ...
  function d3_event_dragSuppress() {
    var name = ".dragsuppress-" + ++d3_event_dragId, click = "click" + name, w = d33.select(d3_window).on("touchmove" + name, d3_eventPreventDefault).on("dragstart" + name, d3_eventPreventDefault).on("selectstart" + name, d3_eventPreventDefault);
  ...
  d33.touch = function(container, touches, identifier) {
  ...
  d33.behavior.drag = function() {
    var event = d3_eventDispatch(drag, "drag", "dragstart", "dragend"), origin = null, mousedown = dragstart(d3_noop, d33.mouse, d3_behavior_dragMouseSubject, "mousemove", "mouseup"), touchstart = dragstart(d3_behavior_dragTouchId, d33.touch, d3_behavior_dragTouchSubject, "touchmove", "touchend");
    function drag() {
      this.on("mousedown.drag", mousedown).on("touchstart.drag", touchstart);
    }
    function dragstart(id, position, subject, move, end) {
      return function() {
        var that = this, target = d33.event.target, parent = that.parentNode, dispatch = event.of(that, arguments), dragged = 0, dragId = id(), dragName = ".drag" + (dragId == null ? "" : "-" + dragId), dragOffset, dragSubject = d33.select(subject()).on(move + dragName, moved).on(end + dragName, ended), dragRestore = d3_event_dragSuppress(), position0 = position(parent, dragId);
        if (origin) {
        ...
        function ended() {
          ...
          dragRestore(dragged && d33.event.target === target);
          dispatch({
            type: "dragend"
          });
        }
  ...
  function d3_behavior_dragTouchSubject() {
    return d33.event.target;
  }
  function d3_behavior_dragMouseSubject() {
    return d3_window;
  }
  d33.touches = function(container, touches) {
  ...
  d33.behavior.zoom = function() {
      ...
      g.each(function() {
        ...
        if (d3_transitionInheritId) {
          d33.select(this).transition().each("start.zoom", function() {
            ...
            var dx = size[0], dy = size[1], cx = center0 ? center0[0] : dx / 2, cy = center0 ? center0[1] : dy / 2, i = d33.interpolateZoom([ (cx - view.x) / view.k, (cy - view.y) / view.k, dx / view.k ], [ (cx - view1.x) / view1.k, (cy - view1.y) / view1.k, dx / view1.k ]);
            return function(t) {
              var l = i(t), k = dx / l[2];
              this.__chart__ = view = {
                x: cx - l[0] * k,
    ...
    function mousedowned() {
      var that = this, target = d33.event.target, dispatch = event.of(that, arguments), dragged = 0, subject = d33.select(d3_window).on(mousemove, moved).on(mouseup, ended), location0 = location(d33.mouse(that)), dragRestore = d3_event_dragSuppress();
    ...
    function touchstarted() {
      var that = this, dispatch = event.of(that, arguments), locations0 = {}, distance0 = 0, scale0, zoomName = ".zoom-" + d33.event.changedTouches[0].identifier, touchmove = "touchmove" + zoomName, touchend = "touchend" + zoomName, targets = [], subject = d33.select(that), dragRestore = d3_event_dragSuppress();
      started();
      zoomstarted(dispatch);
    ...
    function mousewheeled() {
      ...
      if (mousewheelTimer) clearTimeout(mousewheelTimer); else translate0 = location(center0 = center || d33.mouse(this)), 
    ...
    function dblclicked() {
      var p = d33.mouse(this), k = Math.log(view.k) / Math.LN2;
      zoomTo(this, p, location(p), d33.event.shiftKey ? Math.ceil(k) - 1 : Math.floor(k) + 1);
    }
  ...
  function d3_hcl(h, c, l) {
    return this instanceof d3_hcl ? void (this.h = +h, this.c = +c, this.l = +l) : arguments.length < 2 ? h instanceof d3_hcl ? new d3_hcl(h.h, h.c, h.l) : h instanceof d3_lab ? d3_lab_hcl(h.l, h.a, h.b) : d3_lab_hcl((h = d3_rgb_lab((h = d33.rgb(h)).r, h.g, h.b)).l, h.a, h.b) : new d3_hcl(h, c, l);
  }
  ...
  d33.timer = function(callback, delay, then) {
    var n = arguments.length;
    if (n < 2) delay = 0;
  ...
  var d3_format_types = d33.map({
    ...
    r: function(x, p) {
      return (x = d33.round(x, d3_format_precision(x, p))).toFixed(Math.max(0, Math.min(20, d3_format_precision(x * (1 + 1e-15), p))));
    }
  });
  ...
  function d3_time_formatRe(names) {
    return new RegExp("^(?:" + names.map(d33.requote).join("|") + ")", "i");
  }
  ...
  d33.geo.albers = function() {
    return d33.geo.conicEqualArea().rotate([ 96, 0 ]).center([ -.6, 38.7 ]).parallels([ 29.5, 45.5 ]).scale(1070);
  };
  ...
  d33.geo.rotation = function(rotate) {
  ...
  d33.geo.graticule = function() {
    ...
    function lines() {
      return d33.range(Math.ceil(X0 / DX) * DX, X1, DX).map(X).concat(d33.range(Math.ceil(Y0 / DY) * DY, Y1, DY).map(Y)).concat(d33.range(Math.ceil(x0 / dx) * dx, x1, dx).filter(function(x) {
        return abs(x % DX) > ε;
      }).map(x)).concat(d33.range(Math.ceil(y0 / dy) * dy, y1, dy).filter(function(y) {
        return abs(y % DY) > ε;
      }).map(y));
    }
  ...
  function d3_geo_graticuleX(y0, y1, dy) {
    var y = d33.range(y0, y1 - ε, dy).concat(y1);
    return function(x) {
  ...
  function d3_geo_graticuleY(x0, x1, dx) {
    var x = d33.range(x0, x1 - ε, dx).concat(x1);
    return function(y) {
  ...
  d33.geom.quadtree = function(points, x1, y1, x2, y2) {
  ...
  function d3_interpolate(a, b) {
    var i = d33.interpolators.length, f;
    while (--i >= 0 && !(f = d33.interpolators[i](a, b))) ;
    return f;
  }
  d33.interpolators = [ function(a, b) {
    var t = typeof b;
  ...
  d33.transform = function(string) {
    var g = d3_document.createElementNS(d33.ns.prefix.svg, "g");
    return (d33.transform = function(string) {
      if (string != null) {
  ...
  function d3_interpolateTransform(a, b) {
    var s = [], q = [], n, A = d33.transform(a), B = d33.transform(b), ta = A.translate, tb = B.translate, ra = A.rotate, rb = B.rotate, wa = A.skew, wb = B.skew, ka = A.scale, kb = B.scale;
  ...
  d33.layout.chord = function() {
    ...
    function relayout() {
      var subgroups = {}, groupSums = [], groupIndex = d33.range(n), subgroupIndex = [], k, x, x0, i, j;
      chords = [];
      groups = [];
      k = 0, i = -1;
      while (++i < n) {
        ...
        groupSums.push(x);
        subgroupIndex.push(d33.range(n));
        k += x;
      }
      if (sortGroups) {
  ...
  d33.layout.force = function() {
    var force = {}, event = d33.dispatch("start", "tick", "end"), size = [ 1, 1 ], drag, alpha, friction = .9, linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength, charge = -30, chargeDistance2 = d3_layout_forceChargeDistance2, gravity = .1, theta2 = .64, nodes = [], links = [], distances, strengths, charges;
    ...
    force.tick = function() {
      ...
      if (charge) {
        d3_layout_forceAccumulate(q = d33.geom.quadtree(nodes), alpha, charges);
        i = -1;
        while (++i < n) {
          if (!(o = nodes[i]).fixed) {
            q.visit(repulse(o));
    ...
    function dragmove(d) {
      d.px = d33.event.x, d.py = d33.event.y;
      force.resume();
    }
  ...
  d33.layout.pie = function() {
    ...
    function pie(data) {
      ...
      }), a = +(typeof startAngle === "function" ? startAngle.apply(this, arguments) : startAngle), da = (typeof endAngle === "function" ? endAngle.apply(this, arguments) : endAngle) - a, p = Math.min(Math.abs(da) / n, +(typeof padAngle === "function" ? padAngle.apply(this, arguments) : padAngle)), pa = p * (da < 0 ? -1 : 1), k = (da - n * pa) / d33.sum(values), index = d33.range(n), arcs = [], v;
  ...
  var d3_layout_stackOrders = d33.map({
    "inside-out": function(data) {
      var n = data.length, i, j, max = data.map(d3_layout_stackMaxIndex), sums = data.map(d3_layout_stackReduceSum), index = d33.range(n).sort(function(a, b) {
        return max[a] - max[b];
      ...
      if (m > 0) {
        ...
        while (++i < n) {
          ...
          if (x >= range[0] && x <= range[1]) {
            bin = bins[d33.bisect(thresholds, x, 1, m) - 1];
            bin.y += k;
            bin.push(data[i]);
          }
        }
  ...
  d33.layout.treemap = function() {
    var hierarchy = d33.layout.hierarchy(), round = Math.round, size = [ 1, 1 ], padding = null, pad = d3_layout_treemapPadNull, sticky = false, stickies, mode = "squarify", ratio = .5 * (1 + Math.sqrt(5));
  ...
  d33.random = {
    ...
    logNormal: function() {
      var random = d33.random.normal.apply(d3, arguments);
      return function() {
        return Math.exp(random());
      };
    },
    bates: function(m) {
      var random = d33.random.irwinHall(m);
      return function() {
        return random() / m;
      };
    },
  ...
  function d3_scale_polylinear(domain, range, uninterpolate, interpolate) {
    ...
    return function(x) {
      var j = d33.bisect(domain, x, 1, k) - 1;
      return i[j](u[j](x));
    };
  }
  ...
  function d3_scale_linearRebind(scale, linear) {
    return d33.rebind(scale, linear, "range", "rangeRound", "interpolate", "clamp");
  }
  ...
  function d3_scale_linearTicks(domain, m) {
    return d33.range.apply(d3, d3_scale_linearTickRange(domain, m));
  }
  function d3_scale_linearTickFormat(domain, m, format) {
    ...
    if (format) {
      ...
      match.shift();
      if (match[8] === "s") {
        var prefix = d33.formatPrefix(Math.max(abs(range[0]), abs(range[1])));
        if (!match[7]) match[7] = "." + d3_scale_linearPrecision(prefix.scale(range[2]));
        match[8] = "f";
        format = d33.format(match.join(""));
        return function(d) {
  ...
  function d3_scale_log(linear, base, positive, domain) {
    ...
    scale.tickFormat = function(n, format) {
      ...
      if (arguments.length < 2) format = d3_scale_logFormat; else if (typeof format !== "function") format = d33.format(format);
  ...
  d33.scale.sqrt = function() {
    return d33.scale.pow().exponent(.5);
  };
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    function steps(start, step) {
      return d33.range(domain.length).map(function(i) {
        return start + step * i;
      });
    }
    scale.domain = function(x) {
  ...
  function d3_scale_quantile(domain, range) {
    ...
    function rescale() {
      ...
      thresholds = [];
      while (++k < q) thresholds[k - 1] = d33.quantile(domain, k / q);
      return scale;
    }
    function scale(x) {
      if (!isNaN(x = +x)) return range[d33.bisect(thresholds, x)];
    }
    scale.domain = function(x) {
      if (!arguments.length) return domain;
  ...
  function d3_scale_threshold(domain, range) {
    function scale(x) {
      if (x <= x) return range[d33.bisect(domain, x)];
    }
  ...
  d3_transitionPrototype.attr = function(nameNS, value) {
    ...
    var interpolate = nameNS == "transform" ? d3_interpolateTransform : d3_interpolate, name = d33.ns.qualify(nameNS);
    function attrNull() {
      this.removeAttribute(name);
    }
    function attrNullNS() {
  ...
  d3_transitionPrototype.ease = function(value) {
    ...
    if (typeof value !== "function") value = d33.ease.apply(d3, arguments);
  ...
  d33.svg.axis = function() {
    var scale = d33.scale.linear(), orient = d3_svg_axisDefaultOrient, innerTickSize = 6, outerTickSize = 6, tickPadding = 3, tickArguments_ = [ 10 ], tickValues = null, tickFormat_;
    function axis(g) {
      g.each(function() {
        var g = d33.select(this);
        var scale0 = this.__chart__ || scale, scale1 = this.__chart__ = scale.copy();
        var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues, tickFormat = tickFormat_ == null ? scale1.tickFormat ? scale1.tickFormat.apply(scale1, tickArguments_) : d3_identity : tickFormat_, tick = g.selectAll(".tick").data(ticks, scale1), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick").style("opacity", ε), tickExit = d33.transition(tick.exit()).style("opacity", ε).remove(), tickUpdate = d33.transition(tick.order()).style("opacity", 1), tickSpacing = Math.max(innerTickSize, 0) + tickPadding, tickTransform;
  ...
  d33.svg.brush = function() {
    ...
    function brush(g) {
      g.each(function() {
        var g = d33.select(this).style("pointer-events", "all").style("-webkit-tap-highlight-color", "rgba(0,0,0,0)").on("mousedown.brush", brushstart).on("touchstart.brush", brushstart);
        ...
        var gUpdate = d33.transition(g), backgroundUpdate = d33.transition(background), range;
        if (x) {
    ...
    function brushstart() {
      var target = this, eventTarget = d33.select(d33.event.target), event_ = event.of(target, arguments), g = d33.select(target), resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x, resizingY = !/^(e|w)$/.test(resizing) && y, dragging = eventTarget.classed("extent"), dragRestore = d3_event_dragSuppress(), center, origin = d33.mouse(target), offset;
      var w = d33.select(d3_window).on("keydown.brush", keydown).on("keyup.brush", keyup);
  ...
  function d3_time_scale(linear, methods, format) {
    ...
    function tickMethod(extent, count) {
      var span = extent[1] - extent[0], target = span / count, i = d33.bisect(d3_time_scaleSteps, target);
  ...
  var d3_time_scaleMilliseconds = {
    range: function(start, stop, step) {
      return d33.range(Math.ceil(start / step) * step, +stop, step).map(d3_time_scaleDate);
    },
    floor: d3_identity,
    ceil: d3_identity
  };
  ...
  d33.json = function(url, callback) {
    return d3_xhr(url, "application/json", d3_json, callback);
  };
  function d3_json(request) {
    return JSON.parse(request.responseText);
  }
  d33.html = function(url, callback) {
  ...
  if (typeof define === "function" && define.amd) define(d3); else if (typeof module === "object" && module.exports) module.exports = d33;
  this.d33 = d33;
}();